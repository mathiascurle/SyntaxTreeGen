...
int main() {
  ...
  InitWindow(fWinWidth, fWinHeight, "SyntaxTreeGen");
  ...
  std::string sentence;
  std::string currentSentence = "The quick brown fox";
  Typing::s_font = LoadFontEx("Fonts/Rethink_Sans/RethinkSans-Regular.ttf",
                              64,
                              0,
                              0);
  ...
  while (!WindowShouldClose()) {
    ...
    vCurrentWorldPos = GetScreenToWorld2D(vCurrentPos, camera);
    vDeltaPos = Vector2Subtract(vMousePrevPos, vCurrentPos);
    vDeltaPosCamera = Vector2Subtract(GetScreenToWorld2D(vMousePrevPos, camera),
                                      vCurrentWorldPos);
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON) &&
        !CheckCollisionPointRec(vCurrentPos,
                               {fWinWidth - winWidthImGui, 0, fWinWidth,
                                fWinHeight})) { // Mouse pressed not in ImGui
      // Start typing
      ...
      if (!tree.selectNode(vCurrentWorldPos)) {
        tree.setDoingBoxSelect(true);
      }
      // if (!tree.startDraggingNode(GetScreenToWorld2D(vCurrentPos, camera)) &&
      //     !CheckCollisionPointRec(vCurrentPos, {fWinWidth-winWidthImGui, 0,
      //     fWinWidth, fWinHeight}))
      //   tree.resetSelected();

      // Start resizing
      int side = GridSpace::isOverResize(vCurrentWorldPos);
      if (side == 1)
    ...
    if (IsMouseButtonDown(MOUSE_RIGHT_BUTTON) ||
      ...
      camera.target =
          GetScreenToWorld2D(Vector2Add(camera.offset, vDeltaPos), camera);
    } else if (IsMouseButtonDown(
                   MOUSE_LEFT_BUTTON)) { // Mouse left down without left control
      if (!tree.isDoingBoxSelect()) {
        tree.setDraggingNodes(true);
      }
      if (tree.isDraggingNodes()) {
        tree.dragNodes(vDeltaPosCamera);
      } else if (!tree.isDoingBoxSelect()) {
        if (GridSpace::resizing)
          GridSpace::resize(vDeltaPosCamera);
      }
    } else { // Neither mouse left or right is down
      tree.setDraggingNodes(false);
      tree.setDoingBoxSelect(false);
    ...
    if (Typing::isTyping()) {
      ...
      if (IsKeyPressed(KEY_ENTER)) {
        // Typing::setWorkingString(tree.getSelectedNodeString(),
        // tree.getSelectedNodeBounds());
        bIsTypingNode = true;
      }
      // if (IsKeyPressed(KEY_C))
      //   tree.connectSelectedNodes();
      // if (IsKeyPressed(KEY_A))
      //   tree.addPhraseNode(vCurrentWorldPos);
    ...
    {
      GridSpace::draw();
      GridSpace::drawResizeLine(vCurrentWorldPos);
      tree.drawTree();
    ...
    DrawTextEx(Typing::s_font, currentSentence.c_str(),
               Vector2{sentenceRec.x + 10.f,
                       sentenceRec.y + sentenceRec.height - 45.f},
               32, 2, BLACK);
    if (Typing::isTyping() && !bIsTypingNode) {
      Typing::drawCursor();
      Typing::drawSelectedBounds();
    ...
    DrawTextureRec(viewTexture.texture,
    ...
    if (ImGui::Begin("Side bar", &bViewIsOpen, ImGuiWindowFlags_NoTitleBar)) {
      ...
      if (ImGui::CollapsingHeader("Selected phrase")) {
        ...
        if (ImGui::InputText("Phrase type", phraseBuffer,
                             IM_ARRAYSIZE(phraseBuffer))) {
      ...
      if (ImGui::CollapsingHeader("Global vars")) {
        ...
        ImGui::DragFloat("Global Y offest", &SyntaxTree::s_fGlobOffsetY, 1.f,
                         -fWinHeight, fWinHeight);
      }
      if (ImGui::CollapsingHeader("Grid params")) {
        if (ImGui::DragFloat("Grid width", GridSpace::getWidth(), 1.f, 0,
                             2000) ||
            ImGui::DragFloat("Grid height", GridSpace::getHeight(), 1.f, 0,
                             2000))
      ...
      if (ImGui::Button("Save image")) {
        ...
        int i = 0;
        std::string filename = sentence + "_" + to_string(i) + ".png";
